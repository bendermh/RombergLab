	...
	if (status == kIOReturnSuccess) {
        ...
        if (status == -536870174){
            ...
            if([userLanguage isEqualToString:@"es"]){
                [alert setMessageText:@"Ha habido un error al acceder al servicio Bluetooth de su ordenador."];
                [alert setInformativeText:@"Encienda el servicio Bluetooth (Preferencias del Sistema: Bluetooth)"];
            }
            else{
                [alert setMessageText:@"There is an error accessing your computer's Bluetooth service."];
                [alert setInformativeText:@"Turn ON the Bluetooth service (System Preferences: Bluetooth)"];
            }
            [alert addButtonWithTitle:@"OK"];
            [alert runModal];
        }
        // Fin experimento y ES OK !!!
